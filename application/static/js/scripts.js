    ...
    try {
      ...
      if (unreadMsgList.length) {
        ...
        for (u of unreadMsgList) {
          let tmpl = `<li class='border-top'><a class="dropdown-item" href="/user/treatment/${u.patient_username}#patient-conversation">${u.patient_fullname}<br><small>${u.created_at}</small></a></li>`
          menuItems += tmpl;
        }
